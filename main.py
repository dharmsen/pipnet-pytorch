...
from pipnet.utils.args import get_args, save_args, get_optimizer_nn
from pipnet.utils.data import get_dataloaders
from pipnet.utils.eval_cub_csv import get_topk_cub, eval_prototypes_cub_parts_csv, get_proto_patches_cub
from pipnet.utils.func import init_weights_xavier
from pipnet.utils.log import Log
from pipnet.utils.vis_pipnet import visualize_topk, visualize
from pipnet.utils.visualize_prediction import vis_pred, vis_pred_experiments
...
def run_pipnet(args=None):
    ...
    optimizer_net, optimizer_classifier, params_to_freeze, params_to_train, params_backbone = get_optimizer_nn(
        net, args
    )
    ...
    with torch.no_grad():
        if args.state_dict_dir_net != "":
            ...
            try:
                ...
                print(f"Got an error while checkpoint loading: {e=}")
            if 1.0 < torch.mean(net.module._classification.weight).item() < 3.0 and torch.count_nonzero(
                torch.relu(net.module._classification.weight - 1e-5)
            ).float().item() > 0.8 * (
                num_prototypes * len(classes)
    ...
    if net.module._num_classes == 2:
        ...
        print(
            "Your dataset only has two classes. Is the number of samples per class similar?"
            " If the data is imbalanced, we recommend to use the --weighted_loss flag to account for the imbalance.",
            flush=True,
        )
    else:
    ...
    for epoch in range(1, args.epochs_pretrain + 1):
        ...
        for param in params_backbone:
            param.requires_grad = False  # can be set to True when you want to train whole backbone
        ...
        log.log_values(
            "log_epoch_overview", epoch, "n.a.", "n.a.", "n.a.", "n.a.", "n.a.", "n.a.", "n.a.", train_info["loss"]
        )

    if args.state_dict_dir_net == "":
        net.eval()
        torch.save(
            {"model_state_dict": net.state_dict(), "optimizer_net_state_dict": optimizer_net.state_dict()},
    ...
    with torch.no_grad():
        if "convnext" in args.net and args.epochs_pretrain > 0:
            topks = visualize_topk(
                net, projectloader, len(classes), device, "visualised_pretrained_prototypes_topk", args
            )
    ...
    # such that the model can re-active zeroed-out prototypes. Hence an intuitive choice.
    if args.epochs <= 30:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if (epoch == args.epochs or epoch % 30 == 0) and args.epochs > 1:
            ...
            with torch.no_grad():
                ...
                print(
                    "Classifier weights: ",
                    net.module._classification.weight[net.module._classification.weight.nonzero(as_tuple=True)],
                    (net.module._classification.weight[net.module._classification.weight.nonzero(as_tuple=True)]).shape,
                    flush=True,
                )
                if args.bias:
        ...
        with torch.no_grad():
            ...
            if epoch % 30 == 0:
                ...
                torch.save(
                    ...
                    os.path.join(os.path.join(args.log_dir, "checkpoints"), "net_trained_%s" % str(epoch)),
                )
    ...
    if args.dataset == "CUB-200-2011":
        ...
        eval_prototypes_cub_parts_csv(
            csvfile_topk, parts_loc_path, parts_name_path, imgs_id_path, "train_topk_" + str(epoch), args, log
        )

        csvfile_all = get_proto_patches_cub(
            net, projectloader, "train_all_" + str(epoch), device, args, threshold=cubthreshold
        )
        eval_prototypes_cub_parts_csv(
            csvfile_all,
            parts_loc_path,
        ...
        eval_prototypes_cub_parts_csv(
            csvfile_topk, parts_loc_path, parts_name_path, imgs_id_path, "test_topk_" + str(epoch), args, log
        )
        cubthreshold = 0.5
        csvfile_all = get_proto_patches_cub(
            net, test_projectloader, "test_" + str(epoch), device, args, threshold=cubthreshold
        )
        eval_prototypes_cub_parts_csv(
            csvfile_all,
            parts_loc_path,
